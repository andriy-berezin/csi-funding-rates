...
function App() {
  ...
  const [bybitRates, setBybitRates] = useState();
  ...
  const bybitParam = urlParams.get('bybit')
  ...
  const splitFundingRates = ({result}) => {
    ...
    const bybitSymbols = bybit.data.marketPairs.map(i=>i.baseSymbol)
    const bybitRates = topRates.filter(i=>bybitSymbols.includes(i.future.replace("-PERP", "")))
    setFundingRates(finalCoins)
    setLongRates(longRates);
    setShortRates(shortRates);
    setTopRates(topRates)
    setBybitRates(bybitRates)
  };

  const showRows = (rates, top = false, exchange="FTX") => rates.map((element, index) => {
    let future, futureUrlSymbol
    switch (exchange) {
      case "FTX":
        future = element.future
        futureUrlSymbol = element.future.replace('-','')
        break;
      case "BYBIT":
        future = element.future.replace("-PERP", "/USDT")
        futureUrlSymbol = element.future.replace('-PERP','USDT')
        break; 
    }
    ...
    return (
      <tr key={element.future}>
        ...
        >{element.rate}</td>
        <td>
          <a href={`https://www.tradingview.com/chart?symbol=${exchange}:${futureUrlSymbol}`} 
        ...
        <Tabs.Tab label="Top 100 (Bybit)">
            <Container p={0} mr="xl">
              ...
              ( <Table striped highlightOnHover>
                  ...
                  <tbody>{showRows(bybitRates, true, "BYBIT")}</tbody>
                </Table>)}
              </ScrollArea>
            </Container>
        </Tabs.Tab>}
